...
class PostTag(models.Model):
    name = models.CharField(
        ...
        unique = True, 
        verbose_name = _('Tag'), 
        help_text = _('Create tags to categorize your posts')
    )
...
class BlogPage( Page ):
    ...
    DATE_FILTER_CHOICES = [
        (DF_ALL,       _('All')),
        (DF_TODAY,     _('Today')),
        (DF_WEEK,      format_lazy( 'last {d} days', d = DF_WEEK      )),
        (DF_MONTH,     format_lazy( 'last {d} days', d = DF_MONTH     )),
        (DF_TRIMESTER, format_lazy( 'last {d} days', d = DF_TRIMESTER )),
        (DF_SEMESTER,  format_lazy( 'last {d} days', d = DF_SEMESTER  )),
        (DF_YEAR,      format_lazy( 'last {d} days', d = DF_YEAR      )),
    ]
...
class Post(models.Model):
    date = models.DateField(
        ...
        db_index = True, 
        verbose_name = _('Date'), 
        help_text = _('The date of the post')
    )
    ...
    image = models.ImageField(
        ...
        help_text = _('Upload a thumbnail image to display with this post')
    )
    ...
    tags = models.ManyToManyField(
        ...
        related_name = 'posts', 
        verbose_name = _('Tags'), 
        help_text = _('Select all the tags this post is related to.')
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
