...
def showBlog(request, pageId, addFilter=''):
    ...
    if pageFilter:
        myPosts = Post.objects.filter( postTags = pageFilter ).order_by('-postDate')
        tagFiltered = pageFilter.catName
    else:
    ...
    for post in myPosts:
        for tag in post.postTags.all():
            if tag.tagName not in allTags and tag.tagName != tagFiltered:
