...
class ItemCategory(models.Model):
    name = models.CharField(
        ...
        help_text = _('Create categories and later assign your items to one or many the them.')
    )
...
class ItemsPage( Page ):
    ...
    headerText = tmceModels.HTMLField( 
        blank = True, 
        null = True, 
        verbose_name = _('Previous text'), 
        help_text = _('Enter text to be displayed before the items grid (optional)')
    )
    footText = tmceModels.HTMLField( 
        blank = True, 
        null = True, 
        verbose_name = _('Posterior text'), 
        help_text = _('Enter text to be displayed after the items grid (optional)')
    )
    enableShopCart = models.BooleanField( 
        default = False, 
        verbose_name = _('Enable Shopping Cart'), 
        help_text = _('Allow user to add/remove items to a Shopping Cart and later send you an inquiry email attaching the list.')
    )
...
class Item(models.Model):
    code = models.CharField(
        ...
        db_index = True, 
        verbose_name = _('Code'), 
        help_text = _('The code that identifies the item')
    )
    name = models.CharField(
        max_length = 60, 
        verbose_name = _('Name'), 
        help_text = _('Short name of the item')
    )
    descrip = models.TextField(
        blank = True, 
        verbose_name = _('Description'), 
        help_text = _('Description of the item as you want to appear in the site')
    )
    thumb = models.ImageField(
        upload_to = 'ShowItems', 
        verbose_name = _('Thumbnail image'), 
        help_text = _('Thumbnail image that will appear in the items grid.')
    )
    image = models.ImageField(
        upload_to = 'ShowItems', 
        null = True, 
        blank = True, 
        verbose_name = _('Detailed Image'), 
        help_text = _('Large detailed image of the item, designed to cover almost the entire screen')
    )
    categs = models.ManyToManyField(
        ItemCategory, 
        related_name = 'items', 
        verbose_name = _('Categories'), 
        help_text = _('Select all the categories that this item belongs to.')
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
...
class ShoppingCart( models.Model ):
    cartId = models.CharField(
        max_length = 100, 
        unique = True, 
        help_text = 'uuid4 code stored in the session to maintain a separate Shopping Cart for each user browsing the site'
    )
    items = models.ManyToManyField(
        Item, 
        related_name = 'ShoppingCarts',
        help_text = 'The items list of the Cart'
    )
    updated = models.DateTimeField(
        auto_now = True, 
        help_text = 'Last update date, used to automatically clean the unused (abandoned) carts'
    )
